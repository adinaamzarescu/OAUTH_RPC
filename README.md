...
- **`extern ifstream input_file_4`**: Global input file stream used for reading approvals data.
...
- `input_file_4`: File stream for handling approvals.
...
1. **Initialize File Path Variables**:
   - `user_file`, `resources_file`, and `approvals_file` are initialized to `NULL`.

2. **Allocate and Copy Command-Line Arguments**:
   - `allocate_and_copy_string(&user_file, argv[1])`: Allocates memory and copies the first command-line argument to `user_file`.
   - `allocate_and_copy_string(&resources_file, argv[2])`: Allocates memory and copies the second command-line argument to `resources_file`.
   - `allocate_and_copy_string(&approvals_file, argv[3])`: Allocates memory and copies the third command-line argument to `approvals_file`.
...
4. **Open User File**:
   - `OPEN_FILE(input_file_1, user_file)`: Opens the user file using the `OPEN_FILE` macro and assigns it to `input_file_1`.
...
7. **Open Resource File**:
   - `OPEN_FILE(input_file_2, resources_file)`: Opens the resources file using the `OPEN_FILE` macro and assigns it to `input_file_2`.
...
10. **Open Approvals File**:
    - `input_file_4.open(approvals_file)`: Opens the approvals file.
    - `CHECK_FILE(input_file_4, approvals_file)`: Checks if the file opened successfully.

#### Memory Cleanup

11. **Free Allocated Memory**:
    - `FREE_MEMORY(user_file)`: Frees the memory allocated for `user_file`.
    - `FREE_MEMORY(resources_file)`: Frees the memory allocated for `resources_file`.
    - `FREE_MEMORY(approvals_file)`: Frees the memory allocated for `approvals_file`.
